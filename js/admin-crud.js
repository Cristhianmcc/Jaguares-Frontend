...
function actualizarRangoHoras() {
    ...
    if (!inicio || !fin) {
        alert('❌ Debes configurar ambas horas');
        return;
    }
    ...
    categoriasData.forEach(cat => {
        ...
        tbody.innerHTML += `
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
                ...
                <td class="px-6 py-4 text-sm">${cat.deporte}</td>
                <td class="px-6 py-4 text-sm">
                    ${cat.icono ? `<span class="material-symbols-outlined align-middle">${cat.icono}</span>` : ''} 
                    ${cat.nombre}
                </td>
                <td class="px-6 py-4 text-sm">${cat.descripcion || '-'}</td>
...
async function ejecutarEliminarHorario() {
    ...
    try {
        ...
        if (data.success) {
            mostrarNotificacion('✅ Horario desactivado correctamente', 'success');
            cargarHorarios();
            cargarCalendario();
        } else {
...
async function eliminarTodosHorariosDeHora(hora) {
    ...
    try {
        ...
        alert(`✅ ${eliminados} horario(s) eliminados`);
        cargarCalendario();
    } catch (error) {
        alert('❌ Error de conexión');
    }
...
async function guardarEdicionRapida(e) {
    ...
    if (formData.cupo_maximo < horarioActual.cupos_ocupados) {
        alert(`❌ El cupo máximo no puede ser menor que los cupos ocupados (${horarioActual.cupos_ocupados})`);
        return;
    }
    ...
    try {
        ...
        if (data.success) {
            ...
            mostrarNotificacionTemporal('✅ Horario actualizado correctamente');
        } else {
            alert('❌ ' + data.error);
        }
    } catch (error) {
        ...
        if (data.success) {
            ...
            data.categorias.forEach(cat => {
                if (cat.estado === 'activo') {
                    ...
                    option.textContent = `${cat.nombre} (${cat.ano_min}-${cat.ano_max})`;
    ...
    Object.values(porHorario).forEach(grupo => {
        html += `
            <div class="bg-white dark:bg-gray-800 rounded-lg p-6 shadow mb-6 reporte-grupo">
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">Categoría: ${grupo.categoria} | ${grupo.alumnos.length} alumnos</p>
        ...
        ventana.document.write(`
            <div class="grupo">
                ...
                <div class="grupo-info">
                    Categoría: ${grupo.categoria} | Total: ${grupo.alumnos.length} alumno(s)
                </div>
                <table>
                    <thead>
                        <tr>
